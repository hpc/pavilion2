...
class Emitter:
    ...
    def need_more_events(self):
        ...
        # One or more comments may proceed pretty much any event.
        event = None
        offset = 0
    ...
    def need_events(self, count, offset):
        ...
        for event in self.events[offset + 1:]:
            if isinstance(event, (CommentEvent)):
                # Each comment we see increases the total amount of events
                # needed.
                count += 1
    ...
    def expect_comment(self, defer=False):
        ...
        # If the comment is deferred, save the comment for later
        # processing. Returns True if there was a comment.
    ...
    def check_empty_sequence(self):
        if isinstance(self.event, SequenceStartEvent):
            ...
            while (i < len(self.events) and
                ...
                i += 1
            if i < len(self.events) and isinstance(self.events[i],
                                                   SequenceEndEvent):
                return True
        return False
    ...
    def write_comment(self, text):
        """Write a comment at the current indentation level. Force a
        newline before and after, and wrap as appropriate."""

        width = self.best_width - self.indent - 2
        # Use at least half our best width, even if that exceeds our
        # expected width.
        if width < self.best_width/2:
            width = self.best_width/2

        for paragraph in text.splitlines():

            l_para = paragraph.lstrip()
            c_indent = len(paragraph) - len(l_para)
            w_indent = c_indent + 2
            lines = textwrap.wrap(
                l_para,
                width=width,
                initial_indent=' ' * c_indent,
                subsequent_indent=' ' * w_indent)
