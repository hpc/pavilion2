...
def validate_pbs_states(states):
    ...
    for state in states:
        if not re.match('^[a-z-]*$', state):
            raise ValueError("Invalid PBS state '{}'. PBS states should be alpha numeric")
    return states
...
class PBSVars(SchedulerVariables):
    ...
    @dfr_var_method
    def queue(self):
        if self._sched_config['pbs']['queue'] != None:
...
class PBS(SchedulerPluginAdvanced):
   ...
   def _get_alloc_nodes(self, job) -> NodeList:
       ...
       with open(nodefile, 'r') as nf:
           node_list = nf.read().split('\n')

       try:
           return self.parse_node_list(node_list)
       except ValueError as err:
           raise SchedulerPluginError("Invalid pbs nodelist: '{}'".format(node_list), err)
   ...
   def _kickoff(self, pav_cfg, job: Job, sched_config: dict, job_name: str,
       ...
       proc = subprocess.Popen(cmd + ['-o={}'.format(job.sched_log.as_posix()),
   ...
   def _pbsnodes_parse(self, section: str) -> Dict[str, str]:
   ...
   def _job_status(self, pav_cfg, job_info: JobInfo) -> TestStatusInfo:
       ...
       if len(job_data) > 0:
           ...
           return TestStatusInfo(
               ...
               note=("Could not find info on pbs job '{}' in pbs."
                     .format(job_info['id'])),
               when=time.time())
