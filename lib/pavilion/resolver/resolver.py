...
class TestConfigResolver:
    ...
    def load_iter(self, tests: List[str], modes: List[str] = None, overrides: List[str] = None,
        ...
        while raw_tests:
            ...
            while resolved_tests_count < batch_remain and raw_tests:
                ...
                print('megafoo', len(raw_tests), resolved_tests_count, batch_remain)
            ...
            resolved_tests.extend(self._resolve_variables(ready_to_resolve))
            ready_to_resolve = []

            # Finally, return batches of the resolved tests.
            while resolved_tests_count > batch_size:
                print('minifoo', len(resolved_tests), resolved_tests_count, batch_size, resolved_tests_count>batch_size)
        ...
        for request in requests:
            if not request.request_matched and not request.has_error and not raw_tests_empty:
                ...
                if request.seen_subtitles:
                    subtitles = [' - {}'.format(sub) for sub in request.seen_subtitles]
                    subtitles = '\n'.join(subtitles)
                else:
                    subtitles = '  No permutations exist.'
    ...
    def load(self, tests: List[str],
             ...
             conditions=None) -> List[ProtoTest]:
    ...
    def _resolve_variables(self, ptests: ProtoTest) -> List[ProtoTest]:
        """Resolve the variables in all the given tests, in parallel."""
