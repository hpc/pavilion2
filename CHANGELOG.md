...
- Fixed a bug that that caused Spack to issue a traceback due to running in a non-interactive
  context.
- Fixed a bug in which series file result loggers were not writing result logs.
- Fixed a bug in which result logging sometimes created rogue processes. Result logging processes
  now time out after one hour if no new results have been detected. This timeout value can be
  configured by a new `result_logger_timeout` option in the Pavilion config file.
- Log files created by series file loggers are now named with both the series ID and date and time
...
- Added a `--run-data` option to `pav run`, which outputs information about the started test
  series.
- Added the `pav cd` command, which navigates to the test run directory for the given test.
- Added the `pav isolate` command, which isolates tests so that they can be run independently
  of Pavilion for ease of debugging.
- Improved locking for NFS systems.
- Added logging for result loggers.
- Fixed a pair of bugs in the `outliers` expression function which result in dividing by zero
  when passed the empty list or a list in which all values are the same.
- Fixed a bug in which the `command` result parser did not run commands inside the build
  directory'
- Fixed a bug in which host and platform configs were applied in the wrong order.
- Fixed a bug in which the `tasks_per_node` scheduler option was not properly set in generated
  sbatch scripts.
...
## [2.5] - 2026-01-14
 ...
 - Subcommands such as `status` and `cancel` now support filtering on a variety of test attributes
   via the `--filter` argument.
 - Added support for result logger plugins, which allow control over how results are logged. Added
   built-in loggers for logging all results to a single file, as well as for logging each series
   to its own file. This feature addresses a bug in which sufficiently large numbers of tests,
   running simultaneously, would cause the global results lock to fail. The `result_log` key in
   the Pavilion config is now deprecated. Please consult the documentation for instructions on how
   to configure result loggers in the config.
 - Certain subcommands, including `log` and `cat` now automatically use the most recent test or
   series ID rather than requiring it to be explicitly specified.
 - Added config options to automatically perform a module purge when building and running tests.
   This behavior is disabled by default so as not to break existing tests.
 - Replaced `os` configs with `platform` configs, which add support for OS-architecture pairs,
   such as `cos2-x86_64`.
 - Added the ability for users to cancel tests run by other users using `pav cancel`.
 - Fixed a bug in which changes to the `create_files` clause did not cause tests to be rebuilt
   when run a second time.
 - Fixed a bug in which `pav cancel` sometimes canceled the wrong series.
 - Fixed a bug in which tests sometimes showed a status of `SCHED_ERROR` but eventually ran to
   completion.
 - Fixed a bug in which tests would occasionally transition to a `BUILD_CREATED` status after
   failure.
 - Various minor bug fixes for stability.
