...
class SeriesCmdTests(PavTestCase):
    ...
    def test_cancel_series(self):
        ...
        cancel_args = arg_parser.parse_args(['series', 'cancel', str(series_cmd.last_run_series._id)])
    ...
    def test_series_sets(self):
        ...
        sid = str(series_cmd.last_run_series._id)
    ...
    def test_series_list(self):
        ...
        list_args = [
            ['series', 'list'],
            ['series', 'ls', str(series_cmd.last_run_series._id)],
            ['series', 'status', 'all'],
        ]
    ...
    def test_series_history(self):
        ...
        list_args = [
            ...
            ['series', 'states', str(series_cmd.last_run_series._id)],
        ]
        for raw_args in list_args:
            args = arg_parser.parse_args(raw_args)
            self.assertEqual(series_cmd.run(self.pav_cfg, args), 0)
