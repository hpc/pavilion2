...
def execute(function, filenames, verbose, outfile):
    ...
    try:
        ...
        if isinstance(exc, AssertionError):
            ...
            kind = 'ERROR'
        if verbose:
            traceback.print_exc(limit=1, file=outfile)
        else:
            outfile.write(kind[0])
            outfile.flush()
    else:
        kind = 'SUCCESS'
        info = None
...
def display(results, verbose, outfile):
    ...
    for name, filenames, kind, info in results:
        ...
        if kind == 'ERROR':
            traceback.print_exception(file=outfile, *info)
        else:
            outfile.write('Traceback (most recent call last):\n')
            traceback.print_tb(info[2], file=outfile)
            outfile.write('%s: see below\n' % info[0].__name__)
            outfile.write('~'*75+'\n')
            for arg in info[1].args:
                pprint.pprint(arg, stream=outfile)  # ext-print: ignore
        for filename in filenames:
            ...
            if data and data[-1] != '\n':
                outfile.write('\n')
    outfile.write('='*75+'\n')
    outfile.write('TESTS: %s\n' % total)
    if failures:
        outfile.write('FAILURES: %s\n' % failures)
    if errors:
        outfile.write('ERRORS: %s\n' % errors)
    return not (failures or errors)

def run(collections, args=None, outfile=sys.stdout):
            ...
            for base, exts in test_filenames:
                ...
                for ext in function.unittest:
                    ...
                    for skip_ext in skip_exts:
                        ...
                        result = execute(function, filenames, verbose, outfile)
                        results.append(result)
        else:
            result = execute(function, [], verbose, outfile)
            results.append(result)
    return display(results, verbose=verbose, outfile=outfile)
