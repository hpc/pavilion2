...
def test_implicit_resolver(data_filename, detect_filename, verbose=False):
    ...
    try:
        ...
        if verbose:
            print("CORRECT TAG:", correct_tag)  # ext-print: ignore
            if hasattr(node, 'value'):
                print("CHILDREN:")  # ext-print: ignore
                pprint.pprint(node.value)  # ext-print: ignore
...
def test_path_resolver_loader(data_filename, path_filename, verbose=False):
    ...
    try:
        ...
        if verbose:
            print(yaml.serialize_all(nodes1))  # ext-print: ignore
