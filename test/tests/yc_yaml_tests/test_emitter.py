...
def test_emitter_on_canonical(canonical_filename, verbose=False):
    ...
    for canonical in [False, True]:
        ...
        if verbose:
            print("OUTPUT (canonical=%s):" % canonical)  # ext-print: ignore
            print(output)  # ext-print: ignore
        new_events = list(yaml.parse(output))
        _compare_events(events, new_events)
    ...
    for filename in [data_filename, canonical_filename]:
        ...
        for flow_style in [False, True]:
            for style in ['|', '>', '"', '\'', '']:
                ...
                if verbose:
                    print("OUTPUT (filename=%r, flow_style=%r, style=%r)" % (filename, flow_style, style))  # ext-print: ignore
                    print(output)  # ext-print: ignore
                new_events = list(yaml.parse(output))
                _compare_events(events, new_events)
...
def test_emitter_events(events_filename, verbose=False):
    ...
    output = yaml.emit(events)
    if verbose:
        print("OUTPUT:", events_filename)  # ext-print: ignore
        print(output)  # ext-print: ignore
    new_events = list(yaml.parse(output))
