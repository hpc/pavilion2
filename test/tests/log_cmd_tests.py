...
class LogCmdTest(PavTestCase):
    ...
    def test_log_arguments(self):
        ...
        args = parser.parse_args(['run', test.id])
        ...
        args = parser.parse_args(['build', test.id])
        ...
        args = parser.parse_args(['kickoff', test.id])
        result = log_cmd.run(self.pav_cfg, args)
        out, err = log_cmd.clear_output()
    ...
    def test_log_tail(self):
        ...
        args = parser.parse_args(['--tail', '3', 'run', test.id])
        result = log_cmd.run(self.pav_cfg, args)
        self.assertEqual(result, 0)
        out, err = log_cmd.clear_output()
        self.assertEqual(err, '')
    ...
    def test_follow(self):
        ...
        args = parser.parse_args(['--follow', 'run', test.id])
    ...
    def test_log_states(self):
        ...
        for args in (
                ('states', test.id),
                ('states', '--raw', test.id),
                ('states', '--raw_time', test.id),
                ('states', '--raw', '--raw_time', test.id),
                ):
